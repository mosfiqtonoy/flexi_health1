...
def add_health_record(user_id, weight, height, bp_sys, bp_dia, blood_type, notes):
    try:
        ...
        db.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (user_id, weight or None, height or None, bp_sys or None, bp_dia or None, blood_type or None, notes or None)
        )
        db.commit()
        return True
    except Exception:
...
def submit_service_request(user_id, service_type, description, amount_used=0.0):
    try:
        ...
        if amount_used > 0:
            if user.balance < 500:
                return False, "Minimum savings of 500 BDT required to use balance."
            if user.balance < amount_used:
...
def get_dashboard_summary(user_id):
    try:
        db = get_db()
        health_count = db.execute("SELECT COUNT(*) FROM health_records WHERE user_id = ?", (user_id,)).fetchone()[0]
        request_count = db.execute("SELECT COUNT(*) FROM service_requests WHERE user_id = ?", (user_id,)).fetchone()[0]
        recharge_total = db.execute("SELECT COALESCE(SUM(amount), 0) FROM recharge_history WHERE user_id = ?", (user_id,)).fetchone()[0]
        user = User.find_by_id(user_id)
        ...
        return {"health_records": 0, "service_requests": 0, "total_recharged": 0, "balance": 0}
