...
def verify_sslcommerz(data):
    try:
        store_passwd = current_app.config.get("SSLCOMMERZ_STORE_PASSWORD", "")
...
def payment_success():
    try:
        ...
        db.commit()
        flash(f"Payment successful! {savings} BDT saved.", "success")
        
    except Exception as e:
        current_app.logger.error(f"PAYMENT SUCCESS ERROR: {e}")
        flash("Payment received but processing failed.", "danger")
